#' Plotting time series of F/Fmsy and SSB/MSY B trigger by guilds. This plots
#' are used int the Ecosystem Overviews.
#'
...
#'
#' @param x a dataframe output of guild_trends function
...
#' @examples
#' \dontrun{
#' plot1 <- plot_guild_trends(trends2018)
#' }
#'
#' @references
#'
...
plot_guild_trends <- function(x,cap_year, cap_month, return_data = FALSE){
        df<- x
        adj_names <- sort(setdiff(unique(df$FisheriesGuild), "MEAN"))
        ...
        duplicates <- dplyr::filter(duplicates,FisheriesGuild == "MEAN")
        ...
        mean <- dplyr::filter(df, FisheriesGuild == "MEAN")
        df2 <- dplyr::filter(df,FisheriesGuild != "MEAN")
