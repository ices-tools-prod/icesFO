...
load_sag_status <- function(year) {
        ...
        sid <-dplyr::filter(sid,!is.na(YearOfLastAssessment))
        sid <- dplyr::select(sid,StockKeyLabel,
                             YearOfLastAssessment, EcoRegion,AdviceCategory)
        colnames(sid) <- c("StockKeyLabel", "AssessmentYear", "Ecoregion", "AdviceCategory")
        out <- merge(out, sid, by = c("StockKeyLabel", "AssessmentYear"), all = FALSE)
